...
export class CarteComponent implements AfterViewInit {
    ...
    initializeMap(user_id?: number) {
		...
		if (this.checkPermission('manager') && user_id !== undefined && user_id !== 0) {
			this.getScheduleByUserId(user_id);
		} else {
        	this.getSchedule();
		}
    }
    ...
    private getRouteSteps(itineraryData: any): string[] {
        ...
        for (let i = 0; i < stops.length; i++) {
            ...
            routeSteps.push(
                `${currentStop.address} - Arrivée prévue le ${formattedDate}`
            );
        }
    ...
    private plotItinerary(itineraryData: any) {
        ...
        this.itineraryLayer = Leaflet.polyline(path_coordinates, {
            color: 'blue',
        }).addTo(this.map);
    ...
    selectStep(index: number): void {
        ...
        if (
            this.itineraryData &&
            this.itineraryData.path &&
            this.itineraryData.path.points
        ) {
