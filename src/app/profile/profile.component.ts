...
export class ProfileComponent {
  ...
  constructor(private router: Router, private fb: FormBuilder, private securityService: SecurityService, private userService: UserService, private managerService: ManagerService) { 
    ...
    this.entrepriseForm = this.fb.group({
      ...
      address: [localStorage.getItem('addressEntreprise'), [Validators.required]],
    });
    this.getRoles();
  }
  ...
  updateMDP () {
    if (this.formPassword.value.password != this.formPassword.value.passwordConfirm) {
  ...
  updateEntreprise() {
    this.managerService.updateEntreprise(this.entrepriseForm.value).subscribe(
      (res) => {
        console.log(res);
